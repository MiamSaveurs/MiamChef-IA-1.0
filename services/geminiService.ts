...
const getDietaryConstraints = (diet: string): string => {
  switch (diet) {
    ...
    case 'Régime Crétois': 
    case 'Méditerranéen':
        return "RÉGIME CRÉTOIS / MÉDITERRANÉEN : VOUS DEVEZ IMPÉRATIVEMENT GÉNÉRER DES RECETTES DU RÉGIME MÉDITERRANÉEN. La base de TOUS les repas doit être végétale (légumes, légumineuses, noix, céréales complètes). Huile d'olive comme source de graisse principale. VIANDE ROUGE : Maximum 1 à 2 fois par MOIS. VOLAILLE/OEUFS : Modéré (1-2 fois par semaine). POISSON : Modéré (2 fois par semaine). Les autres jours doivent être SANS protéine animale (protéines végétales uniquement).";
...
export const scanFridgeAndSuggest = async (base64Image: string, dietary: string = 'Classique (Aucun)'): Promise<GeneratedContent> => {
  ...
  const textPart = {
    ...
    4. FILTRER selon REGIME : ${dietary} (${dietRules}). C'EST TRÈS IMPORTANT. La recette générée DOIT IMPÉRATIVEMENT respecter ce régime alimentaire, quitte à NE PAS UTILISER certains ingrédients incompatibles identifiés sur la photo. Exemple: si régime Végétarien et que de la viande est visible, IGNOREZ la viande. Adaptez la recette pour le régime spécifié en priorité.
  ...
  const response: GenerateContentResponse = await ai.models.generateContent({
    ...
    config: {
      temperature: 0.2, // lowered temperature for speed and precision
      responseMimeType: "application/json",
      responseSchema: recipeSchema,
    }
  });
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => {
      ...
      img.onload = () => {
        ...
        if (!ctx) return resolve((reader.result as string).split(',')[1]); // Fallback
      ...
      img.onerror = () => resolve((reader.result as string).split(',')[1]); // Fallback
