  ...
  return (
    <div className="pb-32 px-4 pt-6 max-w-3xl mx-auto min-h-screen">
      <header className="mb-8 flex items-center gap-3">
        ...
        <div>
           <h2 className="text-3xl font-display text-chef-dark leading-none">{t('de_title')}</h2>
           <p className="text-gray-500 text-sm font-body">{t('de_sub')}</p>
        </div>
      </header>
            ...
            >
                {originalImage ? (
                    <>
                        ...
                        <div className="absolute bottom-2 left-2 bg-black/60 text-white text-[10px] px-2 py-1 rounded-lg font-bold uppercase backdrop-blur-sm">{t('de_before')}</div>
                    ...
                    <div className="text-center text-gray-400 p-4">
                        ...
                        <span className="text-sm font-display text-purple-500">{t('de_add')}</span>
                    </div>
                )}
            </div>
            ...
            <div className={`aspect-square rounded-2xl flex items-center justify-center overflow-hidden relative border-2 ${editedImage ? 'border-chef-green ring-2 ring-green-100' : 'border-dashed border-gray-200 bg-gray-50'}`}>
                {status === 'loading' ? (
                    <div className="text-center">
                       ...
                       <span className="text-xs text-purple-500 font-bold animate-pulse">{t('loading')}</span>
                    </div>
                ) : editedImage ? (
                    <>
                        <img src={editedImage} alt="Edited" className="w-full h-full object-cover" />
                        <div className="absolute bottom-2 right-2 bg-chef-green text-white text-[10px] px-2 py-1 rounded-lg font-bold uppercase shadow-sm">{t('de_after')}</div>
                    ...
                    <div className="text-center text-gray-400 p-4">
                        ...
                        <span className="text-sm font-display">{t('de_result')}</span>
                    </div>
                )}
            </div>
        </div>
            ...
            <div>
                ...
                <div className="relative">
                    <input
                        ...
                        placeholder={t('de_prompt_placeholder')}
                        value={prompt}
                ...
                <div className="mt-3 flex items-center gap-1.5 justify-center text-[10px] text-gray-400">
                    <ShieldCheck size={10} /> {t('de_privacy')}
                </div>
            </div>
        </div>
      </div>
